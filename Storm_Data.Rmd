...
###### EXHIBIT 5: Bar Plots representing Total Damages & Fatalities for Top 10 Event Type
...
###### EXHIBIT 6: Plot results for top 10 Event Types, by Year
...
event.adj <- merge(economic.mean, economic.max, by = "EVENT_TYPE")
event.adj <- select(event.adj, EVENT_TYPE, TTLDMG.x, TTLDMG.y)
event.adj <- setnames(event.adj, old = c("EVENT_TYPE", "TTLDMG.x", "TTLDMG.y"),
                                    new = c("EVENT_TYPE", "AVG_DMG", "MAX_DMG"))

event.dmg.adj <- merge(event.dmg, event.adj, by = "EVENT_TYPE")
event.dmg.adj$TTLDMG_ADJ <- ifelse(event.dmg.adj$TTLDMG - event.dmg.adj$MAX_DMG < 0,
                                    event.dmg.adj$TTLDMG, event.dmg.adj$AVG_DMG)
