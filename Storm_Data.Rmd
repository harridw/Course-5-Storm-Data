...
###### Economic Consequences (Total Damage in Millions / Average Damage in Thousands)
...
```{r economic_consequences_report, include = TRUE}
economic.report.smry
```
...
###### Event Hazard to Population Health
...
```{r event_hazard_report, include = TRUE}
pophlth.sum
```
...
##### Plot results for each of the top 10 Event Types
```{r plot_event_damage, fig.keep = "all", fig.show = "asis", fig.path = 'figure/'}
...
theme_update(plot.title = element.text(hjust = 0.5))
ggplot(event.dmg10, aes(x=BGN_YEAR, y=DMG_AMT, group=EVENT_TYPE)) +
      geom_line() +
      geom_point() +
      labs(title = "Total Damage for Top 10 Events") +
